...
The profile uses an original midnight-blue, ice-blue, and cyan visual system: a typographic hero with a decorative code editor, compact project covers, and matching repository-hosted statistics. It does not load JavaScript, remote fonts, or third-party statistics images in the README.
...
All profile artwork lives in `assets/profile/` and is generated by `scripts/build_artwork.mjs` using Node.js 18+ with no dependencies. There are fourteen SVGs: desktop and mobile variants of the hero, four project covers, the toolkit, and the contact panel.

```bash
node scripts/build_artwork.mjs
```

- **Hero:** large typography, a subtle grid and blue glow, and a decorative code editor with a gently pulsing cursor. The name and positioning stay still.
- **Projects:** compact covers with distinct accent colors, category labels, technology lists, and decorative code symbols. These are not product screenshots or measured data.
- **Motion:** a slow CSS opacity pulse embedded inside the hero SVG. No JavaScript, SMIL, external images, remote fonts, or third-party rendering services are required.
...
Edit the generator, then regenerate the artwork. Palette constants are inside `buildArtwork()` in `scripts/build_artwork.mjs`; statistics colors are in `scripts/update_stats.py`. Keep changes to both consistent so scheduled statistics updates preserve the design.
